...
- [Вопросы к экзамену](#вопросы-к-экзамену)
  - [Базовые сведения об ОС. Процессы и потоки](#базовые-сведения-об-ос-процессы-и-потоки)
  ...
  - [Файловые системы и динамические библиотеки](#файловые-системы-и-динамические-библиотеки)
  ...
  - [Компьютерные сети](#компьютерные-сети)
    ...
    - [Модели взаимодействия: клиент–сервер и P2P](#модели-взаимодействия-клиентсервер-и-p2p)
    - [Интернет-сервисы. e-commerce, интернет вещей](#интернет-сервисы-e-commerce-интернет-вещей)
    - [Типы сетей в современной экосистеме интернета. Корпоративные сети](#типы-сетей-в-современной-экосистеме-интернета-корпоративные-сети)
    - [Сети по масштабу: PAN, LAN, MAN, WAN, Internetworks](#сети-по-масштабу-pan-lan-man-wan-internetworks)
    - [Интернет: устройство и история](#интернет-устройство-и-история)
    - [Мобильные и беспроводные сети](#мобильные-и-беспроводные-сети)
    - [Сетевые протоколы: цели, надёжность, ресурсы, QoS. Архитектура  сетевых уровней](#сетевые-протоколы-цели-надёжность-ресурсы-qos-архитектура--сетевых-уровней)
    - [Службы: соединение/без соединения, надёжность и примитивы](#службы-соединениебез-соединения-надёжность-и-примитивы)
    - [Эталонные сетевые модели. Модель OSI](#эталонные-сетевые-модели-модель-osi)
    - [Модель TCP/IP: происхождение, структура и логика разделения уровней](#модель-tcpip-происхождение-структура-и-логика-разделения-уровней)
    - [Стандартизация в сетях](#стандартизация-в-сетях)
    - [Физический уровень. Проводные среды](#физический-уровень-проводные-среды)
    - [Беспроводная связь и электромагнитный спектр](#беспроводная-связь-и-электромагнитный-спектр)
    - [Цифровая передача через аналоговый канал. Мультиплексирование](#цифровая-передача-через-аналоговый-канал-мультиплексирование)
    - [Канальный уровень: фреймы, границы сообщений, ошибки и подтверждения](#канальный-уровень-фреймы-границы-сообщений-ошибки-и-подтверждения)
    - [Управление потоком. Обнаружения ошибок](#управление-потоком-обнаружения-ошибок)
    - [Протоколы канального уровня. Конвейерная передача](#протоколы-канального-уровня-конвейерная-передача)
    - [Подуровень MAC. Ethernet](#подуровень-mac-ethernet)
    - [Роль сетевого уровня. Коммутация пакетов](#роль-сетевого-уровня-коммутация-пакетов)
    - [Службы сетевого уровня для транспортного уровня. Дейтаграммные службы](#службы-сетевого-уровня-для-транспортного-уровня-дейтаграммные-службы)
    - [Виртуальные каналы и дейтаграммы](#виртуальные-каналы-и-дейтаграммы)
    - [Маршрутизация внутри автономной системы. Принцип оптимальности Беллмана](#маршрутизация-внутри-автономной-системы-принцип-оптимальности-беллмана)
    - [Алгоритм Дейкстры. Лавинная адресация](#алгоритм-дейкстры-лавинная-адресация)
    - [Вектор расстояний и алгоритм Беллмана–Форда. Проблема счёта до бесконечности](#вектор-расстояний-и-алгоритм-беллманафорда-проблема-счёта-до-бесконечности)
    - [Принципы проектирования сетевого уровня интернета (RFC 1958)](#принципы-проектирования-сетевого-уровня-интернета-rfc-1958)
    - [Транспортный уровень и его службы](#транспортный-уровень-и-его-службы)
    - [Примитивы транспортной службы. Клиент-серверная модель](#примитивы-транспортной-службы-клиент-серверная-модель)
    - [Сокеты Беркли. Клиент и сервер](#сокеты-беркли-клиент-и-сервер)
    - [Анализ примера файлового сервера/клиента](#анализ-примера-файлового-сервераклиента)
    - [Элементы транспортных протоколов. Адресация](#элементы-транспортных-протоколов-адресация)
    - [Установление и разрыв соединения](#установление-и-разрыв-соединения)
    - [Прикладной уровень. Основы DNS](#прикладной-уровень-основы-dns)
    - [DNS. иерархия имён, управление доменами, резолвинг](#dns-иерархия-имён-управление-доменами-резолвинг)
    - [DNS: записи (RR), зоны и обратный поиск, безопасность и диагностика](#dns-записи-rr-зоны-и-обратный-поиск-безопасность-и-диагностика)
    - [Электронная почта](#электронная-почта)
    - [Всемирная паутина. Браузер. Веб сервер.](#всемирная-паутина-браузер-веб-сервер)
    - [Протоколы HTTP и HTTPS](#протоколы-http-и-https)
    - [Передача аудио и видео. Сети доставки контента. P2P](#передача-аудио-и-видео-сети-доставки-контента-p2p)

## Базовые сведения об ОС. Процессы и потоки
...
## Файловые системы и динамические библиотеки
...
- Дайте определение компьютерной сети и объясните, почему важно слово «автономных» устройств. ⭐
- Какие типы сред передачи данных вы знаете, и как выбор среды влияет на скорость, дальность и стоимость связи? ⭐⭐
- Почему интернет часто называют «сетью сетей»? Приведите пример объединения сетей разных масштабов. ⭐⭐
- Объясните, чем отличается «компьютерная сеть» от «распределённой системы» (в общих чертах, без формальных определений). ⭐⭐
- Какие задачи решают сети помимо передачи файлов между компьютерами? Приведите 5–7 примеров. ⭐
- Почему развитие сетей тесно связано с эволюцией вычислительной техники и хранением данных? ⭐⭐
- Какие ключевые технологические сдвиги (18–21 века) подготовили почву для появления компьютерных сетей? ⭐
- Почему в XX веке «информация» стала ключевым ресурсом, и как это связано с сетями? ⭐⭐
- Объясните, как изменился смысл «вычислительного центра» от ранних ЭВМ до современных дата-центров. ⭐⭐
- Какие основные компромиссы (trade-offs) чаще всего приходится учитывать при проектировании сетей? ⭐⭐⭐
- Почему ранние компьютеры были централизованными, и какие ограничения такой модели стали критичными? ⭐⭐
- Какие факторы привели к «отказу от одного большого ЭВМ» и переходу к сети автономных устройств? ⭐⭐
- Что значит «распределение вычислительных задач» в контексте сетей? Приведите пример. ⭐⭐
- Почему объединение множества компьютеров в сеть может быть эффективнее одного очень мощного компьютера? ⭐⭐
- В каких случаях централизованная модель всё ещё выигрывает у распределённой (приведите 2–3 ситуации)? ⭐⭐⭐

### Модели взаимодействия: клиент–сервер и P2P

- Опишите клиент–серверную модель так, чтобы было понятно, какие роли у клиента и у сервера. ⭐
- Почему веб-браузер — типичный пример клиента? Какие задачи он «не обязан» делать сам? ⭐⭐
- За счёт чего один сервер может обслуживать тысячи клиентов одновременно? Назовите хотя бы 3 механизма/идеи. ⭐⭐⭐
- Какие слабые места есть у клиент–серверной архитектуры (отказоустойчивость, масштабирование, безопасность)? ⭐⭐
- Что такое одноранговая (P2P) сеть и чем она принципиально отличается от клиент–серверной? ⭐
- Объясните идею BitTorrent на уровне принципов: почему разбиение на части ускоряет распространение файла? ⭐⭐
- Почему P2P может быть устойчивее к отказам, чем централизованные системы? ⭐⭐
- Почему P2P часто сложнее в администрировании и контроле (технически и организационно)? ⭐⭐⭐
- Почему электронную почту иногда называют частично P2P системой? Какие элементы здесь «централизованы», а какие — «распределены»? ⭐⭐⭐
- Приведите пример сервиса, который может использовать гибридную модель (центральная координация + P2P), и объясните зачем. ⭐⭐⭐

### Интернет-сервисы. e-commerce, интернет вещей

- Почему электронная почта стала «массовым и универсальным инструментом», и какие сетевые свойства для этого критичны? ⭐⭐
- Чем мессенджеры по сетевой природе отличаются от email (режимы доставки, задержки, сессии, присутствие)? ⭐⭐
- Почему потоковое видео доминирует в трафике, и какие требования это накладывает на сети доступа и магистрали? ⭐⭐
- Какие отличия в требованиях к сети у видеоконференции и у просмотра видео «по запросу»? ⭐⭐⭐
- Какие сетевые проблемы чаще всего «видит» пользователь (буферизация, лаги, разрывы), и какие причины стоят за ними? ⭐⭐
- Что такое электронная коммерция и какие типы взаимодействий (B2C, B2B, C2C, G2C, P2P) вы можете объяснить на примерах? ⭐
- Почему онлайн-аукционы часто похожи на P2P модель даже при наличии центрального сайта? ⭐⭐
- Какие сетевые риски характерны для финансовых операций (подмена, перехват, повтор сообщений), и как их обычно предотвращают? ⭐⭐⭐
- Что такое m-commerce и почему мобильная среда меняет пользовательское поведение оплаты? ⭐⭐
- Как работает NFC на концептуальном уровне и какие требования к безопасности здесь особенно важны? ⭐⭐⭐
- Дайте определение IoT и объясните, чем IoT-устройства отличаются от обычных компьютеров с точки зрения сети. ⭐⭐
- Какие типичные ограничения есть у IoT-устройств (энергия, вычисления, память) и как они отражаются на сетевых протоколах? ⭐⭐⭐
- Приведите примеры «умных» счётчиков и объясните, почему для них важны надёжность и безопасность. ⭐⭐
- Что такое сенсорная сеть и чем она отличается от «обычной» беспроводной сети смартфонов? ⭐⭐
- Почему миниатюризация датчиков меняет сценарии мониторинга (животные, транспорт, парковки)? ⭐⭐
- Какие угрозы безопасности наиболее критичны для IoT (в сравнении с ПК), и почему? ⭐⭐⭐

### Типы сетей в современной экосистеме интернета. Корпоративные сети

- Опишите «цепочку» прохождения трафика от пользователя до контента: доступ → транзит → дата-центр/CDN (в общем виде). ⭐⭐
- Что такое сеть широкополосного доступа и почему её называют «последние метры»? ⭐
- В чём смысл закона Меткалфа и как его можно интерпретировать применительно к интернету и соцсетям? ⭐⭐
- Сравните xDSL, коаксиал и оптоволокно как среду доступа: плюсы/минусы и типовые сценарии. ⭐⭐
- Почему в некоторых регионах мобильная сеть становится основным домашним доступом? Какие последствия это имеет? ⭐⭐
- Что такое сеть дата-центра и почему внутри неё возникают «огромные объёмы трафика между серверами»? ⭐⭐
- Почему «древовидная» архитектура дата-центров плохо масштабируется? Объясните, что такое узкое место в топологии. ⭐⭐⭐
- Что такое CDN и почему размещение контента «ближе к пользователю» ускоряет загрузку? ⭐⭐
- По каким критериям выбирают «лучшую реплику» в CDN (и почему критерии могут конфликтовать)? ⭐⭐⭐
- Что такое транзитная (магистральная) сеть и когда она используется? ⭐⭐
- Что такое peering и почему он меняет роль транзитных сетей? ⭐⭐
- Почему транзит часто становится «резервным вариантом»? Какие экономические и технические причины? ⭐⭐⭐
- Зачем организации нужна корпоративная сеть, если «есть интернет»? Назовите ключевые мотивы. ⭐⭐
- Какие ресурсы чаще всего «делят» в корпоративной сети и почему это выгодно? ⭐
- Почему корпоративные сети критичны для непрерывности бизнеса? Приведите пример простоя и его последствий. ⭐⭐
- Что такое VPN и как он создаёт «виртуальную частную сеть» поверх публичного интернета? ⭐⭐
- Какие ограничения и риски есть у VPN по сравнению с выделенными линиями? ⭐⭐⭐
- Что такое SD-WAN и чем принципиально отличается «программно управляемая WAN» от классической? ⭐⭐⭐
- Объясните, что такое SLA в контексте WAN и как SD-WAN помогает его соблюдать. ⭐⭐⭐
- Почему в корпоративной среде часто доминирует клиент–серверная модель? ⭐⭐
- Какие типы трафика в корпоративной сети требуют разных приоритетов (почта, VoIP, видео, документы) и почему? ⭐⭐⭐

### Сети по масштабу: PAN, LAN, MAN, WAN, Internetworks

- Сравните PAN, LAN, MAN, WAN по радиусу действия и типичным технологиям. ⭐⭐
- Почему Bluetooth стал основой PAN, и что означает модель «master–slave» в общих чертах? ⭐⭐
- Какие свойства обычно характеризуют LAN (задержки, скорость, ошибки) и почему география так важна? ⭐⭐
- Чем беспроводная LAN отличается от проводной с точки зрения помех, безопасности и управления средой? ⭐⭐
- Объясните разницу между инфраструктурным Wi-Fi и ad hoc режимом. Где какой уместен? ⭐⭐
- Что такое mesh-сеть в Wi-Fi и в каких случаях она лучше, чем «одна точка доступа»? ⭐⭐
- Почему Ethernet стал доминирующим стандартом проводных LAN? Назовите причины «технические» и «экономические». ⭐⭐
- Как коммутатор принимает решение, куда переслать кадр? Почему это отличается от маршрутизатора? ⭐⭐
- Зачем нужны механизмы против зацикливания (например, Spanning Tree) в Ethernet-сетях? ⭐⭐⭐
- Что такое VLAN и как «логическое разделение» повышает управляемость и безопасность? ⭐⭐
- Как исторически возникли MAN и почему кабельное ТВ сыграло важную роль? ⭐
- Почему WAN «соединяет сети, а не компьютеры», и как это упрощает объединение разных технологий? ⭐⭐
- Что такое internetwork и почему маршрутизатор — ключевое устройство интерсети? ⭐⭐
- Какие сложности возникают при соединении разнородных сетей (размеры пакетов, форматы кадров, адресация)? ⭐⭐⭐

### Интернет: устройство и история

- Почему интернет не принадлежит одной организации, и какие последствия это имеет для управления и стандартов? ⭐⭐
- Какие общие протоколы и сервисы «склеивают» интернет как систему (приведите примеры)? ⭐
- Как контекст холодной войны повлиял на требования к устойчивости связи? ⭐⭐
- Почему иерархическая телефония была уязвима с точки зрения отказоустойчивости? ⭐⭐
- В чём идея распределённой сети Пола Бэрана и почему коммутация пакетов здесь принципиальна? ⭐⭐⭐
- Опишите архитектурную идею ARPANET: зачем отделили хосты от подсети передачи (IMP)? ⭐⭐⭐
- Что такое store-and-forward и какие плюсы/минусы у такого принципа передачи? ⭐⭐⭐
- Почему появление разных сетей (спутниковых, радиосетей) потребовало нового подхода к протоколам? ⭐⭐
- В чём ключевая идея TCP/IP как стека для интерсетей? ⭐⭐
- Почему включение TCP/IP в Berkeley Unix и появление сокетов резко ускорили развитие сетевых приложений? ⭐⭐
- Какую роль сыграл NSFNET в масштабировании интернета в научной среде, и что изменилось с появлением коммерческих операторов? ⭐⭐⭐
- Почему WWW стал «поворотным моментом» и как он изменил аудиторию и нагрузку на сеть? ⭐⭐
- Как изменение трафика (от текста к видео) влияет на архитектуру и экономику интернета? ⭐⭐⭐
- Объясните «плоскую» архитектуру современного интернета: почему контент-провайдеры и ISP стремятся к прямым соединениям? ⭐⭐⭐

### Мобильные и беспроводные сети

- Почему архитектура мобильных сетей «сильно отличается от интернета»? Назовите ключевые причины. ⭐⭐
- Что такое радиодоступ (E-UTRAN) и пакетное ядро (EPC) на уровне функций? ⭐⭐
- Почему переход к «полностью пакетной передаче данных» стал важным этапом (особенно в 4G/5G)? ⭐⭐
- Что такое handover и почему без него мобильная сеть не может работать? ⭐⭐
- Чем «жёсткий» handover отличается от «мягкого», и как это влияет на качество связи? ⭐⭐⭐
- Зачем мобильной сети нужно отслеживать местоположение абонента и какую роль играет HSS? ⭐⭐⭐
- Как SIM-карта участвует в безопасности: какие задачи она решает концептуально? ⭐⭐
- Почему шифрование радиоканала обязательно, и какие угрозы оно снижает? ⭐⭐
- Сравните поколения 1G–5G: какие изменения были ключевыми (не только скорость, но и сервисы/архитектура)? ⭐⭐
- Что такое MIMO и почему оно помогает одновременно скорости и надёжности в беспроводной среде? ⭐⭐⭐
- Почему стандартизация 802.11 была необходима для массового распространения Wi-Fi? ⭐⭐
- Что значит «нелицензируемые диапазоны ISM» и почему это одновременно плюс и минус? ⭐⭐
- Объясните проблему multipath fading (многолучевое замирание) простыми словами и её последствия для связи. ⭐⭐⭐
- Какие подходы относятся к path diversity и почему «разнесение» помогает? ⭐⭐⭐
- Почему в беспроводной среде возникает проблема конфликтов передач и чем CSMA/CA отличается по смыслу от «простого Ethernet»? ⭐⭐⭐
- Что такое hidden node и почему он опасен именно для Wi-Fi? ⭐⭐⭐
- Как обеспечивается «переключение» между точками доступа, и почему мобильность Wi-Fi ограничена по сравнению с сотовыми сетями? ⭐⭐⭐
- Почему WEP оказался уязвим, и какие идеи лежат в основе более современных подходов (WPA2/802.11i/802.1X)? ⭐⭐⭐

### Сетевые протоколы: цели, надёжность, ресурсы, QoS. Архитектура  сетевых уровней

- Зачем вообще нужны сетевые протоколы, если «можно просто отправлять биты по проводу»? ⭐⭐
- Какие виды ошибок встречаются в сетях и почему «ошибки неизбежны»? ⭐⭐
- Что такое CRC и как в общих чертах работает обнаружение ошибок? ⭐⭐
- Чем исправление ошибок отличается от повторной передачи (ARQ) и когда выгоднее одно, а когда другое? ⭐⭐⭐
- Что означает «маршрутизация ищет рабочий путь» и почему это часть надёжности? ⭐⭐
- Что такое статистическое мультиплексирование и почему оно эффективно по сравнению с фиксированным выделением ресурсов? ⭐⭐⭐
- В чём разница между flow control и congestion control? Приведите пример, когда нужна каждая из них. ⭐⭐⭐
- Что такое QoS и почему разные приложения предъявляют разные требования к задержке и пропускной способности? ⭐⭐
- Почему «перегрузка сети» может привести к коллапсу производительности, даже если физические каналы не сломались? ⭐⭐⭐
- Зачем делить сеть на уровни и как это помогает эволюции технологий? ⭐⭐
- Объясните понятия peer, протокол и интерфейс на одном примере (например, транспортный уровень). ⭐⭐⭐
- Почему данные физически не «ходят горизонтально» между одинаковыми уровнями на разных хостах? ⭐⭐
- Приведите аналогию «философы–переводчики–секретари» и поясните, что она показывает в сетях. ⭐⭐
- Что такое инкапсуляция в сетях и какую роль играют заголовки? ⭐⭐
- Какие типы информации могут быть в заголовках разных уровней (адреса, номера, длины, метки времени) и зачем они нужны? ⭐⭐⭐
- Что такое фрагментация и почему ограничения нижнего уровня вынуждают её делать? ⭐⭐
- Какие риски и сложности вносит фрагментация (потери фрагментов, сборка, задержки)? ⭐⭐⭐
- Что такое «архитектура сети» и чем она отличается от «стека протоколов» на конкретном узле? ⭐⭐
- Почему интерфейсы и протоколы стандартизируются, а реализация уровней может быть разной? ⭐⭐
- Какие уровни чаще реализуются аппаратно, а какие программно, и почему? ⭐⭐
- Объясните разницу между «службой» и «протоколом» так, чтобы было понятно, почему их смешение вредно. ⭐⭐⭐

### Службы: соединение/без соединения, надёжность и примитивы

- В чём разница между connection-oriented и connectionless службами (по смыслу и по последствиям для сети)? ⭐⭐
- Почему надёжность — отдельная характеристика и не равна «наличию соединения»? ⭐⭐
- Чем отличаются надёжные message sequences и надёжные byte streams с точки зрения приложения? ⭐⭐⭐
- Почему ненадёжная служба может быть предпочтительнее для VoIP/видео, даже если «теряются данные»? ⭐⭐
- Как ненадёжная доставка может стать основой надёжной (идея надстройки уровнем выше)? ⭐⭐
- Что такое «примитивы службы» и почему их удобно воспринимать как системные вызовы? ⭐⭐
- Объясните смысл примитивов LISTEN, CONNECT, ACCEPT: какие состояния они создают и зачем. ⭐⭐⭐
- Почему простого «запрос–ответ» часто недостаточно в реальной сети? Перечислите минимум 3 сетевые проблемы. ⭐⭐
- Зачем нужны номера сообщений/последовательности и как они помогают отличать «старые» данные от «новых»? ⭐⭐⭐
- Какие ресурсы «держит» соединение и почему это важно для масштабируемости серверов? ⭐⭐⭐
- Опишите полный жизненный цикл типичного соединения клиент–сервер: от ожидания до завершения, включая обмен данными. ⭐⭐

### Эталонные сетевые модели. Модель OSI

- Что такое многоуровневая архитектура в сетях и какие проблемы она решает по сравнению с «монолитной» системой? ⭐
- Почему сетевую систему удобнее проектировать как набор уровней, а не как один большой протокол? ⭐⭐
- Что означает принцип «каждый уровень скрывает детали реализации от верхних уровней» и какие преимущества он даёт? ⭐⭐
- В чём разница между архитектурой сети и конкретной реализацией протоколов на устройстве? ⭐⭐
- Какие риски возникают, если уровни начинают «нарушать границы» и выполнять чужие функции? ⭐⭐⭐
- Почему сетевые модели называют эталонными, и чем эталонная модель отличается от реального стека протоколов? ⭐⭐
- Что такое «интероперабельность» и почему она невозможна без согласованной модели взаимодействия? ⭐⭐
- Приведите пример, как многоуровневый подход позволяет заменить одну технологию (например, физическую среду) без переписывания приложений. ⭐⭐
- Почему эталонные модели полезны даже тогда, когда они не реализованы «буквально» в реальных сетях? ⭐⭐
- Перечислите 7 уровней OSI и объясните, почему порядок уровней именно такой (а не произвольный). ⭐⭐
- Чем отличается «служба» уровня от «протокола» уровня, и почему это принципиально важно? ⭐⭐⭐
- Что такое интерфейс уровня в OSI, и чем он отличается от протокола, который работает «между узлами»? ⭐⭐⭐
- Почему OSI считается «чистой архитектурой», и в чём выражается эта чистота? ⭐⭐
- Какие функции OSI запрещает «спускать вниз» и почему это делает систему более управляемой? ⭐⭐⭐
- Объясните, как OSI помогает анализировать проблему «где именно произошёл сбой» в сети. ⭐⭐
- Почему в OSI выделены отдельные сеансовый и представительский уровни, и какие задачи они должны были решать? ⭐⭐⭐
- Какие реальные функции в современных сетях частично «заменяют» сеансовый и представительский уровни OSI? ⭐⭐⭐
- Почему OSI удобно использовать как язык общения инженеров, даже если интернет работает на TCP/IP? ⭐⭐
- Приведите пример ситуации, где путаница между «службой» и «протоколом» приводит к неверному проектному решению. ⭐⭐⭐

### Модель TCP/IP: происхождение, структура и логика разделения уровней

- Почему TCP/IP называют практической моделью, а OSI — теоретической? ⭐⭐
- Какие инженерные задачи ARPANET напрямую повлияли на дизайн TCP/IP? ⭐⭐
- В чём заключается роль IP как «сердца» интерсети, и почему он специально делает доставку ненадёжной? ⭐⭐⭐
- Почему транспортный уровень выделен отдельно от межсетевого, и какие проблемы он закрывает? ⭐⭐
- Чем принципиально отличается доставка «между хостами» от доставки «между процессами»? ⭐⭐⭐
- Почему прикладной уровень TCP/IP получился «слишком большим», и какие последствия это имеет? ⭐⭐
- Объясните, почему TCP/IP проще OSI по структуре, но сложнее по архитектурной строгости. ⭐⭐⭐
- Как TCP/IP обеспечивает совместимость разных сетевых технологий (Ethernet, Wi-Fi, оптика, радиоканалы)? ⭐⭐
- Почему модель TCP/IP плохо подходит для некоторых технологий (например, Bluetooth), и что именно «не ложится» в уровни? ⭐⭐⭐
- Как историческая инерция влияет на развитие TCP/IP и затрудняет замену старых протоколов? ⭐⭐⭐
- Почему OSI проиграла конкуренцию TCP/IP, несмотря на архитектурную строгость? ⭐⭐
- Какие недостатки OSI проявились именно при попытке внедрения «в железе и софте»? ⭐⭐⭐
- В чём состоит критика TCP/IP как модели (не как набора протоколов), и почему это важно? ⭐⭐
- Почему отсутствие разделения «физический/канальный» в TCP/IP считается проблемой? ⭐⭐
- Зачем в книге используется гибридная модель из 5 уровней и какие компромиссы она фиксирует? ⭐⭐
- Какие преимущества даёт гибридная модель при обучении и анализе сетевых технологий? ⭐⭐
- Объясните, какие функции OSI «сжимаются» в прикладной уровень TCP/IP и какие риски это создаёт. ⭐⭐⭐
- Как бы вы объяснили разницу OSI и TCP/IP человеку, который считает, что это «одно и то же»? ⭐
- Почему наличие эталонной модели не гарантирует успеха технологии на рынке? ⭐⭐
- Приведите пример современной технологии, где подход «строго по OSI» был бы неудобен или неэффективен. ⭐⭐⭐

### Стандартизация в сетях

- Почему без стандартизации рынок сетевых технологий быстро превращается в «зоопарк несовместимых решений»? ⭐⭐
- Чем стандарт «определяет совместимость», но не определяет реализацию? Приведите пример. ⭐⭐
- В чём разница между стандартом де-факто и де-юре, и как один может превращаться в другой? ⭐⭐
- Почему открытый исходный код ускоряет принятие стандартов, даже если стандарт формально ещё не утверждён? ⭐⭐⭐
- Как стандарты снижают стоимость оборудования и увеличивают доступность технологий для пользователей? ⭐⭐
- Почему стандартизация важна не только для «железа», но и для протоколов приложений (например, веба)? ⭐⭐
- Какие проблемы появляются, если стандарт слишком строгий и «не оставляет места» для развития? ⭐⭐⭐
- Почему иногда выгоднее иметь один «неидеальный» стандарт, чем несколько «идеальных», но несовместимых? ⭐⭐⭐
- Какую роль играет ITU в сетях и почему распределение радиочастот — критически важная задача? ⭐⭐
- Почему рекомендации ITU-T фактически становятся глобальными стандартами, даже если их «никто не обязан» внедрять? ⭐⭐
- Объясните, как стандарт X.509 связан с безопасностью интернета и почему он относится к телеком-стандартам. ⭐⭐⭐
- Чем подход ISO к стандартизации отличается от подхода IEEE с точки зрения скорости и практичности? ⭐⭐
- Почему стандарты IEEE 802 (Ethernet, Wi-Fi) стали настолько успешными? Назовите причины технические и рыночные. ⭐⭐⭐
- Что означает принцип IETF «приблизительный консенсус и работающий код», и почему он оказался эффективнее бюрократии? ⭐⭐⭐
- Что такое RFC и почему формат RFC стал ключевым инструментом развития интернета? ⭐⭐
- Чем отличается роль IRTF от IETF, и почему важно разделять исследования и инженерную стандартизацию? ⭐⭐
- Почему W3C важен для интернета, хотя «интернет работает на IP и TCP»? ⭐⭐
- Как коммерческие альянсы (например, Wi-Fi Alliance, 3GPP) дополняют классические организации стандартизации? ⭐⭐⭐

### Физический уровень. Проводные среды

- Почему физический уровень считается фундаментом сети и какие характеристики канала он задаёт для всех верхних уровней? ⭐⭐
- Объясните разницу между пропускной способностью (bandwidth) и скоростью передачи данных (bitrate) на практических примерах. ⭐⭐⭐
- Почему задержка передачи данных не может быть меньше фундаментального ограничения скорости света? ⭐⭐
- Как выбор среды передачи влияет на архитектуру сети (например, на необходимость подтверждений и повторов)? ⭐⭐⭐
- Почему «перенеси-и-доставь» (ленты/диски/грузовики) иногда эффективнее передачи по сети? ⭐⭐
- Как сравнить каналы по эффективности, если один имеет огромную скорость, но большую задержку (спутник), а другой — наоборот? ⭐⭐⭐
- Почему витая пара скручивается, и какую физическую проблему это решает? ⭐⭐
- Чем отличается simplex, half-duplex и full-duplex, и как это влияет на производительность сети? ⭐⭐
- Почему категории кабеля (Cat5e, Cat6, Cat8) важны именно с точки зрения частот и помех, а не «маркетинга»? ⭐⭐⭐
- Чем коаксиальный кабель принципиально отличается от витой пары по защите от помех и полосе пропускания? ⭐⭐
- Почему передача данных по электропроводке возможна, но считается «непредсказуемой» средой? ⭐⭐⭐
- Как работает оптоволокно на принципе полного внутреннего отражения и почему это даёт преимущества? ⭐⭐
- В чём отличие одномодового и многомодового волокна и как это влияет на дальность и скорость? ⭐⭐⭐
- Что такое хроматическая дисперсия и почему она ограничивает скорость оптоволоконных линий? ⭐⭐⭐
- Почему оптоволокно сложнее перехватить, но при этом оно более требовательно к монтажу? ⭐⭐

### Беспроводная связь и электромагнитный спектр

- Объясните связь частоты и длины волны и почему инженеру связи важно понимать формулу λ·f = c. ⭐⭐
- Почему более высокая частота обычно означает больший потенциальный канал по скорости, но хуже проходимость? ⭐⭐⭐
- Почему радиосвязь удобна из-за всенаправленности и почему это одновременно её главный недостаток? ⭐⭐
- Чем микроволновая связь отличается от радиосвязи на низких частотах по требованиям к антеннам и условиям распространения? ⭐⭐⭐
- Почему инфракрасная связь считается более «локальной и защищённой», но не универсальной? ⭐⭐
- Какие преимущества и ограничения есть у лазерной связи «здание-здание», и почему погода становится сетевой проблемой? ⭐⭐⭐
- В чём идея расширения спектра и почему она повышает устойчивость к помехам и обнаружению? ⭐⭐⭐
- Как FHSS помогает против глушения и почему он был важен для военной связи? ⭐⭐
- Почему Bluetooth исторически использовал FHSS и какие проблемы диапазона ISM это решает? ⭐⭐⭐
- Как DSSS позволяет нескольким пользователям делить одну и ту же полосу частот (идея CDMA)? ⭐⭐⭐
- Почему GPS и 3G можно считать примерами применения DSSS/CDMA-подходов? ⭐⭐
- Что такое UWB и почему он почти не мешает узкополосным системам при правильной мощности? ⭐⭐⭐

### Цифровая передача через аналоговый канал. Мультиплексирование

- Почему канал передаёт «волны», а мы хотим передавать «биты», и какие два шага нужны для этого преобразования? ⭐⭐
- Почему узкая полоса пропускания «размывает» резкие переходы сигнала и ломает цифровую передачу? ⭐⭐⭐
- Сформулируйте смысл теоремы Найквиста: что именно ограничивает скорость в идеальном канале без шума? ⭐⭐⭐
- Сформулируйте смысл теоремы Шеннона: как шум и SNR создают абсолютный предел скорости? ⭐⭐⭐
- Почему нельзя «обойти» предел Шеннона просто более умным кодированием без изменения полосы и SNR? ⭐⭐⭐
- Чем ASK, FSK и PSK отличаются по физическому смыслу и устойчивости к шуму? ⭐⭐
- Почему QAM позволяет передавать больше бит на символ, но требует лучшего качества канала? ⭐⭐⭐
- Почему NRZ плохо подходит для длинных серий одинаковых битов и к каким сбоям это приводит? ⭐⭐
- Как Манчестерский код решает проблему синхронизации и почему за это приходится платить? ⭐⭐⭐
- Зачем нужны 4B/5B и 8B/10B и что означает «гарантированные переходы» в потоке? ⭐⭐⭐
- Что такое скремблирование и почему оно полезно даже без изменения общей скорости передачи? ⭐⭐
- Почему мультиплексирование — это ключевая идея для повышения эффективности использования физического канала? ⭐⭐
- Сравните FDM и TDM: какие условия делают каждый из методов более выгодным? ⭐⭐⭐
- Почему CDMA можно считать «одновременной передачей всех сразу» и что делает её возможной? ⭐⭐⭐
- Чем WDM отличается от FDM и почему именно в оптоволокне он даёт огромный выигрыш? ⭐⭐

### Канальный уровень: фреймы, границы сообщений, ошибки и подтверждения

- Почему канальный уровень работает с фреймами, а не с битами, и что это даёт верхним уровням? ⭐⭐
- Какие функции канального уровня критичны для «скрытия проблем физического канала» от сетевого уровня? ⭐⭐
- Объясните назначение Header, Payload и Trailer во фрейме и какие ошибки они помогают предотвратить. ⭐⭐
- Чем отличается служба без подтверждений от службы с подтверждениями на канальном уровне и почему Ethernet выбрал первый вариант? ⭐⭐⭐
- Почему Wi-Fi использует подтверждения на канальном уровне, а классический Ethernet — нет? ⭐⭐⭐
- В каких случаях оправдана ориентированная на соединение служба канального уровня и почему она нужна на «плохих» каналах? ⭐⭐⭐
- Почему физический уровень сам по себе не может гарантировать границы сообщений и что из этого следует? ⭐⭐
- Почему метод подсчёта байтов уязвим и как одна ошибка может разрушить синхронизацию потока? ⭐⭐⭐
- Как работает флаговый байт + байт-стаффинг и почему он применим к произвольным данным? ⭐⭐
- Чем бит-стаффинг принципиально лучше байт-стаффинга в некоторых протоколах WAN/USB? ⭐⭐⭐
- Что такое «запрещённые сигналы» физического уровня и почему они могут быть идеальными маркерами границ? ⭐⭐⭐
- Почему подтверждение (ACK) само может потеряться и какие ошибки поведения это создаёт в протоколе? ⭐⭐
- Зачем нужны таймеры на канальном уровне и почему без них протокол может «зависнуть навсегда»? ⭐⭐
- Почему повторная передача без нумерации фреймов приводит к дублированию данных на сетевом уровне? ⭐⭐⭐
- Как нумерация фреймов позволяет отличить повтор от нового сообщения? ⭐⭐
- Почему в stop-and-wait достаточно 1 бита номера (0/1) и в каких условиях этого уже не хватает? ⭐⭐⭐

### Управление потоком. Обнаружения ошибок

- Почему потери могут происходить даже в «идеальном безошибочном канале», если нет управления потоком? ⭐⭐
- Чем feedback-based управление потоком отличается от rate-based, и почему второе чаще относят к транспортному уровню? ⭐⭐⭐
- Какие признаки в реальной сети могут указывать на переполнение буферов получателя, а не на шум в канале? ⭐⭐⭐
- Почему полностью устранить ошибки физического канала слишком дорого и приходится компенсировать их протоколами? ⭐⭐
- В чём принципиальная разница между обнаружением ошибок и исправлением ошибок? ⭐⭐
- Почему FEC особенно полезна на спутниковых и радиоканалах, где повторная передача «дорогая»? ⭐⭐⭐
- Что такое расстояние Хэмминга и как оно определяет возможности кода по обнаружению и исправлению ошибок? ⭐⭐⭐
- Почему бит чётности почти «бесплатный», но слабый метод обнаружения ошибок? ⭐⭐
- Почему контрольные суммы могут иметь «слепые зоны» и какие ошибки они могут не заметить? ⭐⭐⭐
- Почему CRC считается почти идеальным компромиссом для канального уровня? ⭐⭐⭐
- Объясните идею CRC через «деление многочленов по модулю 2». ⭐⭐⭐

### Протоколы канального уровня. Конвейерная передача

- Почему «утопический» протокол полезен как учебная модель, но бесполезен в реальных сетях? ⭐
- Как stop-and-wait решает проблему переполнения буфера и почему он резко снижает эффективность на больших задержках? ⭐⭐⭐
- Какие события должен обрабатывать канальный уровень в реальном протоколе (frame_arrival, timeout, checksum_err) и почему? ⭐⭐⭐
- Что такое ARQ и почему без него надёжная доставка невозможна в ненадёжном канале? ⭐⭐
- Почему piggybacking повышает эффективность и какие риски появляются, если долго нет исходящих данных? ⭐⭐⭐
- Почему stop-and-wait плохо использует канал при большом bandwidth×delay и как это исправляет sliding window? ⭐⭐⭐
- Что означает «размер окна» и как он связан с количеством неподтверждённых фреймов «в полёте»? ⭐⭐
- Почему при sliding window отправителю нужны буферы, и что произойдёт без буферизации? ⭐⭐⭐
- В чём разница между Go-Back-N и Selective Repeat и как они отражают компромисс «простота vs эффективность»? ⭐⭐⭐
- Почему Selective Repeat требует более строгих правил выбора размера окна и нумерации? ⭐⭐⭐
- Как сетевой уровень получает данные «строго по порядку», даже если канальный уровень принимает фреймы неупорядоченно? ⭐⭐⭐

### Подуровень MAC. Ethernet

- Почему широковещательная среда превращает передачу данных в задачу «координации доступа»? ⭐⭐
- Чем статическое распределение канала (TDM/FDM) плохо подходит для всплескового компьютерного трафика? ⭐⭐⭐
- Почему ALOHA имеет низкую эффективность и что означает «уязвимый интервал» для кадра? ⭐⭐⭐
- Почему slotted ALOHA эффективнее чистой ALOHA и за счёт чего достигается выигрыш? ⭐⭐⭐
- В чём идея CSMA и почему «слушать канал перед передачей» принципиально улучшает ситуацию? ⭐⭐
- Чем отличаются 1-настойчивый, ненастойчивый и p-настойчивый CSMA по задержкам и вероятности коллизий? ⭐⭐⭐
- Как работает CSMA/CD и почему прекращение передачи при коллизии экономит ресурс канала? ⭐⭐⭐
- Почему протоколы без коллизий (token, битовая карта) дают предсказуемые задержки и где это критично? ⭐⭐⭐
- Объясните идею двоичного обратного отсчёта и почему его эффективность может приближаться к 100%. ⭐⭐⭐
- Почему классический Ethernet был «общим каналом», а современный Ethernet стал коммутируемым? ⭐⭐
- Зачем в Ethernet нужны преамбула и SFD и что будет, если приёмник не синхронизируется? ⭐⭐⭐
- Почему минимальный размер Ethernet-фрейма связан с обнаружением коллизий и задержкой распространения? ⭐⭐⭐
- Почему в Ethernet нет подтверждений доставки и какие уровни берут на себя восстановление потерь? ⭐⭐⭐
- Как коммутатор обучается таблице MAC→порт и почему flooding считается нормальным поведением? ⭐⭐
- Почему в коммутируемом Ethernet исчезают коллизии и становится возможным полный дуплекс? ⭐⭐
- За счёт чего Ethernet смог увеличивать скорости на порядки, не меняя базовую модель адресации? ⭐⭐
- Какие новые «узкие места» появляются в высокоскоростных Ethernet-сетях (буферы, магистрали, задержки коммутаторов)? ⭐⭐⭐

### Роль сетевого уровня. Коммутация пакетов

- Объясните, какие задачи решает сетевой уровень и почему без него невозможен интернет как «сеть сетей». ⭐⭐
- Почему сетевой уровень называют уровнем доставки «от конца до конца», и чем это принципиально отличается от канального уровня? ⭐⭐
- Что означает «доставка пакетов через множество маршрутизаторов», и какие новые проблемы появляются по сравнению с передачей в одной LAN? ⭐⭐
- Объясните разницу между маршрутизацией и пересылкой (forwarding). Почему это не одно и то же? ⭐⭐
- Почему сетевой уровень должен знать топологию сети, хотя канальный уровень может работать без знания «всей картины»? ⭐⭐
- Что такое принцип end-to-end и почему он стал основой архитектуры интернета? ⭐⭐⭐
- В каких ситуациях принцип end-to-end оказывается неудобным или недостаточным? Приведите примеры. ⭐⭐⭐
- Почему сеть на уровне IP считается «ненадёжной», и какие последствия это имеет для транспортного уровня? ⭐⭐
- Какие виды информации «важнее всего» для принятия решения маршрутизатором при пересылке пакета? ⭐⭐
- Почему избыточная связность (несколько путей между узлами) полезна для устойчивости сети, но усложняет маршрутизацию? ⭐⭐⭐
- Объясните принцип store-and-forward и почему он исторически стал основой пакетной коммутации. ⭐⭐
- Какие задержки формируются при store-and-forward на одном маршрутизаторе, и как они накапливаются на пути? ⭐⭐⭐
- Чем store-and-forward отличается от «прозрачной» передачи битов (как в физическом канале), и почему это важно? ⭐⭐
- Какие преимущества даёт промежуточное хранение пакета в маршрутизаторе (например, для проверки ошибок/управления очередями)? ⭐⭐
- Какие недостатки у store-and-forward для трафика реального времени и почему они проявляются сильнее при перегрузке? ⭐⭐⭐

### Службы сетевого уровня для транспортного уровня. Дейтаграммные службы

- Какие требования предъявляются к службам сетевого уровня со стороны транспортного (независимость от топологии, единая адресация и т.д.)? ⭐⭐
- Почему важно, чтобы сетевой уровень был независим от технологий маршрутизаторов и типа сети (LAN/WAN)? ⭐⭐
- Объясните, что значит «единая система адресации для LAN и WAN» и почему это критично для масштабируемости интернета. ⭐⭐
- В чём заключается исторический спор: «сеть должна быть надёжной» vs «хосты обеспечивают надёжность»? ⭐⭐⭐
- Почему телефонный подход (надёжность и QoS в сети) логичен для голоса и видео? ⭐⭐
- Почему IP-подход (best effort) оказался более жизнеспособным для глобального интернета? ⭐⭐⭐
- Какие функции управления потоком и надёжностью переносит на себя транспортный уровень в IP-сетях? ⭐⭐
- Объясните, почему свобода реализации служб важна для развития сетевых технологий и приложений. ⭐⭐
- Как технологии вроде MPLS и VLAN позволяют добавить QoS поверх IP, не ломая сам принцип best effort? ⭐⭐⭐
- Что такое дейтаграммная сеть и почему в ней каждый пакет маршрутизируется независимо? ⭐⭐
- Почему дейтаграммная модель не требует предварительной настройки пути, и какие преимущества это даёт при сбоях? ⭐⭐
- Какое влияние оказывает независимость пакетов на порядок доставки и вероятность потерь? ⭐⭐
- Объясните, почему в дейтаграммной сети пакеты одного сообщения могут идти разными маршрутами. ⭐⭐
- В каких случаях разный маршрут пакетов одного потока — преимущество, а в каких — проблема? ⭐⭐⭐
- Почему IP считается базовым примером дейтаграммного сервиса и что означает best effort на практике? ⭐⭐
- Какие ограничения у дейтаграммной службы для приложений реального времени (видео/голос)? ⭐⭐

### Виртуальные каналы и дейтаграммы

- Что такое виртуальный канал на сетевом уровне и какие состояния должны хранить маршрутизаторы? ⭐⭐⭐
- Почему виртуальный канал требует времени на установление соединения и как это влияет на короткие транзакции? ⭐⭐
- Что такое VC ID и зачем он нужен в пакетах виртуального канала? ⭐⭐
- Почему виртуальные каналы упрощают обработку пакетов в маршрутизаторах? ⭐⭐
- Объясните, как виртуальные каналы позволяют резервировать ресурсы и обеспечивать QoS. ⭐⭐⭐
- Почему виртуальные каналы более уязвимы к отказам маршрутизаторов по сравнению с дейтаграммами? ⭐⭐⭐
- В каких сценариях виртуальные каналы выгоднее дейтаграмм (например, VPN, долгие сессии)? ⭐⭐
- Почему MPLS можно считать примером «соединений поверх IP» и какие задачи он решает у провайдеров? ⭐⭐⭐
- Сравните дейтаграммный и виртуально-канальный подходы по устойчивости к отказам и объясните причину различий. ⭐⭐⭐
- Почему дейтаграммные сети требуют больших таблиц маршрутизации и более сложной логики выбора следующего хопа? ⭐⭐
- Почему заголовки в дейтаграммной модели обычно «тяжелее», чем в виртуальных каналах? ⭐⭐
- Как изменится поведение сети при перегрузке в модели виртуальных каналов и в модели дейтаграмм? ⭐⭐⭐
- Почему виртуальные каналы легче контролировать с точки зрения QoS, но сложнее масштабировать глобально? ⭐⭐⭐
- Какой тип сервиса вы бы выбрали для коротких запросов (например, веб-запросов), и почему? ⭐⭐
- Какой тип сервиса вы бы выбрали для длительной потоковой передачи с гарантиями, и почему? ⭐⭐

### Маршрутизация внутри автономной системы. Принцип оптимальности Беллмана

- Что такое автономная система (AS) и почему маршрутизация внутри AS отличается от междоменной маршрутизации? ⭐⭐
- Какие требования предъявляются к алгоритмам маршрутизации внутри AS при сбоях и изменениях топологии? ⭐⭐⭐
- Почему маршрут может выбираться «на пакет» или «на сеанс», и как это связано с дейтаграммами и VC? ⭐⭐
- Объясните компромисс между эффективностью и справедливостью при выборе маршрутов. ⭐⭐⭐
- Почему «максимальная пропускная способность сети» может конфликтовать с интересами отдельных пользователей? ⭐⭐⭐
- Чем статическая маршрутизация отличается от динамической, и когда статическая может быть оправдана? ⭐⭐
- Почему динамическая маршрутизация неизбежно приводит к периодам нестабильности (конвергенции)? ⭐⭐⭐
- Сформулируйте принцип оптимальности Беллмана и объясните, почему он важен для маршрутизации. ⭐⭐⭐
- Почему из принципа оптимальности следует идея входного дерева (sink tree) для одного получателя? ⭐⭐
- Что означает «любой участок оптимального пути тоже оптимален» и как это помогает строить маршруты? ⭐⭐⭐
- Почему в реальной сети вместо дерева может получиться DAG, и что это означает для маршрутизации? ⭐⭐⭐
- Почему отсутствие циклов в модели sink tree критично для гарантии доставки за конечное число шагов? ⭐⭐
- Почему sink tree — идеальная модель, но не всегда достижима при меняющейся топологии? ⭐⭐⭐

### Алгоритм Дейкстры. Лавинная адресация

- Почему сеть удобно представлять взвешенным графом, и что именно выступает вершинами и рёбрами? ⭐⭐
- Какие метрики могут использоваться как веса рёбер (хопы, задержка, стоимость) и как они меняют результат маршрутизации? ⭐⭐⭐
- Почему алгоритм Дейкстры требует неотрицательных весов и что произойдёт при нарушении этого условия? ⭐⭐⭐
- Объясните идею меток расстояний в алгоритме Дейкстры и что означает «заморозить» узел. ⭐⭐
- Почему Дейкстра вычисляет кратчайшие пути «от источника ко всем», и как это используется в маршрутизаторах? ⭐⭐
- Как меняется качество маршрутов, если веса отражают задержку, но нагрузка сети быстро меняется? ⭐⭐⭐
- Объясните алгоритм flooding и почему он не требует таблиц маршрутизации. ⭐⭐
- Почему flooding без ограничений приводит к лавинообразному росту дубликатов и перегрузке сети? ⭐⭐
- Как TTL/hop count ограничивает flooding и почему это важно для предотвращения «вечных» пакетов? ⭐⭐
- Как фильтрация дубликатов по паре «источник + порядковый номер» уменьшает нагрузку сети? ⭐⭐
- Почему flooding гарантирует доставку до всех достижимых узлов и в каких сценариях это полезно? ⭐⭐
- Почему flooding может служить «эталоном минимума задержки» и как его использовать для сравнения алгоритмов? ⭐⭐⭐

### Вектор расстояний и алгоритм Беллмана–Форда. Проблема счёта до бесконечности

- Объясните принцип маршрутизации по вектору расстояний и какие данные хранятся в таблице маршрутизатора. ⭐⭐
- Почему маршрутизатору достаточно знать стоимость до соседей, чтобы пересчитывать маршруты до всех узлов? ⭐⭐⭐
- Как периодический обмен векторами расстояний приводит к конвергенции сети к кратчайшим путям? ⭐⭐
- Почему этот алгоритм считается распределённым и какие плюсы это даёт для устойчивости? ⭐⭐
- Какие недостатки возникают из-за того, что маршрутизатор доверяет данным соседей? ⭐⭐⭐
- В чём заключается проблема «счёта до бесконечности» и почему она появляется именно в distance-vector алгоритмах? ⭐⭐⭐
- Почему «хорошие новости» распространяются быстро, а «плохие новости» — медленно? ⭐⭐⭐
- Почему ввод конечного значения «бесконечности» — это компромисс, а не полное решение? ⭐⭐⭐
- Как split horizon уменьшает вероятность зацикливания маршрутов и в каких случаях он не спасает? ⭐⭐⭐
- Что такое poisoned reverse и почему он помогает только частично? ⭐⭐⭐

### Принципы проектирования сетевого уровня интернета (RFC 1958)

- Почему протокол считается «готовым» только после успешных взаимных тестов реализаций? ⭐⭐⭐
- Почему интернет-архитектура предпочитает простые решения и один способ реализации вместо множества опций? ⭐⭐⭐
- Как модульная архитектура стеков протоколов помогает развитию интернета без «переписывания всего»? ⭐⭐
- Почему «параметры должны согласовываться, а не зашиваться» и как это связано с MTU/настройками сети? ⭐⭐⭐
- Объясните принцип «строгий при отправке, снисходительный при приёме» и какие плюсы/риски он несёт. ⭐⭐⭐
- Почему масштабируемость и стоимость/производительность считаются ключевыми ограничениями при проектировании сетевого уровня? ⭐⭐⭐
- Почему интернет можно рассматривать как множество соединённых автономных систем, а не как одну «единую сеть»? ⭐⭐
- Как роль дата-центров и магистралей Tier-1 влияет на современную структуру интернет-трафика? ⭐⭐
- Почему IP оставляет выбор маршрута и балансировку нагрузки задачей маршрутизаторов, а не приложений? ⭐⭐⭐

### Транспортный уровень и его службы

- Объясните, почему транспортный уровень называют уровнем доставки данных **«от приложения к приложению»**, а не «от хоста к хосту». ⭐
- В чём принципиальная разница между задачами сетевого уровня и транспортного уровня? ⭐
- Какие детали нижних уровней транспортный уровень «прячет» от приложений и почему это важно для разработки ПО? ⭐⭐
- Почему транспортный уровень считается частью «ядра» стека вместе с сетевым уровнем? ⭐⭐
- Что значит «транспортный уровень работает на конечных устройствах», и почему это влияет на архитектуру протоколов? ⭐⭐
- Объясните смысл утверждения: «нижние уровни — поставщик, верхние — потребитель транспортной службы». ⭐⭐
- Какие свойства передачи данных могут быть обеспечены транспортным уровнем поверх ненадёжной сети? ⭐⭐
- В каких ситуациях приложениям может быть достаточно сетевого уровня без транспортного — и почему это редкость? ⭐⭐⭐
- Что означает «служба с установлением соединения» и какие фазы она обычно включает? ⭐
- В чём преимущества и недостатки службы без установления соединения по сравнению с соединением? ⭐⭐
- Почему транспортный уровень часто рассматривают как «поставщика надёжности», даже если сеть может быть ненадёжной? ⭐⭐
- Объясните, что означает «идеальный поток данных» с точки зрения приложения. ⭐⭐
- Почему «надёжный поток байтов» — это абстракция, а не реальное свойство сети? ⭐⭐
- Какие компромиссы возникают при попытке сделать передачу «идеально надёжной»? ⭐⭐⭐
- Почему одни приложения выбирают TCP, а другие — UDP, даже если оба работают поверх IP? ⭐⭐

### Примитивы транспортной службы. Клиент-серверная модель

- Зачем транспортному уровню нужен простой набор примитивов (LISTEN, CONNECT, SEND, RECEIVE, DISCONNECT)? ⭐
- Объясните, что именно делает LISTEN на стороне сервера и почему он не равен ACCEPT. ⭐⭐
- Что означает «CONNECT блокирует клиента», и почему блокировка бывает полезной? ⭐⭐
- Какие действия скрыты внутри SEND/RECEIVE, хотя приложение их не видит? ⭐⭐
- Чем отличаются симметричный и асимметричный разрыв соединения на уровне примитивов? ⭐⭐
- Почему транспортный уровень должен корректно работать даже в «маловероятных сценариях»? ⭐⭐
- Какие ошибки проектирования протокола возможны, если игнорировать редкие сценарии потерь и задержек? ⭐⭐⭐
- Опишите пошагово установление соединения в модели клиент–сервер через LISTEN и CONNECT. ⭐⭐
- Какую роль играют управляющие сегменты CONNECTION REQUEST и CONNECTION ACCEPTED? ⭐⭐
- Почему в описании транспорта важна идея «сегмент → IP-пакет → фрейм канального уровня»? ⭐⭐
- Что может произойти, если сервер обрабатывает соединения строго последовательно (один за другим)? ⭐⭐
- Как транспортный уровень помогает реализовать многопользовательские сервисы (много клиентов одновременно)? ⭐⭐⭐

### Сокеты Беркли. Клиент и сервер

- Что такое сокет и почему его можно считать «универсальным API транспорта»? ⭐
- Почему сокеты стали стандартом и чем это полезно для переносимости программ? ⭐⭐
- Объясните смысл последовательности SOCKET → BIND → LISTEN → ACCEPT на сервере. ⭐⭐
- Зачем нужен BIND и что произойдёт, если сервер его не выполнит явно? ⭐⭐
- Что такое очередь входящих соединений (backlog) и как она влияет на поведение сервера при нагрузке? ⭐⭐
- Почему ACCEPT возвращает **новый** сокет, а не использует «слушающий» сокет для обмена данными? ⭐⭐
- Какие ресурсы ОС расходует сервер при большом количестве входящих подключений? ⭐⭐⭐
- Почему клиент обычно не делает BIND вручную, и что делает ОС вместо него? ⭐⭐
- Что значит, что CONNECT устанавливает полнодуплексное соединение? ⭐
- Почему SEND/RECEIVE могут быть эквивалентны READ/WRITE в некоторых ОС? ⭐⭐
- В чём отличие CLOSE как системного вызова от «логического завершения протокола»? ⭐⭐
- Чем отличается работа сокетов в TCP и в UDP с точки зрения поведения CONNECT? ⭐⭐
- Для чего нужны SENDTO и RECEIVEFROM, и почему они важны для UDP? ⭐⭐
- Почему сокет-API подходит даже для новых протоколов (SCTP, QUIC), хотя они отличаются от TCP? ⭐⭐⭐

### Анализ примера файлового сервера/клиента
...
int main(int argc, char* argv[]) {
  ...
  channel.sin_family = AF_INET; // Семейство адресов: IPv4
  channel.sin_addr.s_addr = htonl(INADDR_ANY); // Принимать соединения на всех локальных интерфейсах
  channel.sin_port = htons(8080); // Порт сервера (8080), приводим к сетевому порядку байт

  s = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP); // Создаем TCP-сокет (IPv4, потоковый, протокол TCP)
  if (s < 0) { printf("сбой вызова сокета"); exit(-1); }
  setsockopt(s, SOL_SOCKET, SO_REUSEADDR, (char*)&on, sizeof(on)); // Разрешаем переиспользовать адрес/порт

  b = bind(s, (struct sockaddr*)&channel, sizeof(channel)); // Привязываем сокет к адресу (IP + порт), чтобы ядро знало, какой порт слушать
  if (b < 0) { printf("сбой связывания"); exit(-1); }

  l = listen(s, QUEUE_SIZE); // Переводим сокет в режим прослушивания входящих соединений 
  if (l < 0) { printf("сбой ожидания"); exit(-1); }
  while (1) { // Основной серверный цикл: бесконечно принимаем и обслуживаем подключения
    sa = accept(s, 0, 0); // accept блокируется, пока не придет новое входящее соединение, возвращает сокет, по которому идёт обмен данными
    if (sa < 0) { printf("сбой доступа"); exit(-1); }
    read(sa, buf, BUF_SIZE);
    fd = open(buf, O_RDONLY);
    if (fd < 0) { printf("сбой открытия файла"); exit(-1); }

    while (1) {
      bytes = read(fd, buf, BUF_SIZE);
      if (bytes <= 0) break;
      write(sa, buf, bytes); // Отправляем прочитанные байты клиенту. Предполагается, что write отправит все bytes за один вызов
    }
    close(fd);
    close(sa);
  }
...
    struct sockaddr_in channel;
    if (argc != 3) {
        printf("Для запуска введите: client имя_сервера имя_файла");
        exit(-1);
    }
    h = gethostbyname(argv[1]); // Преобразуем имя хоста (например, flits.cs.vu.nl) в IP-адрес через DNS
    if (!h) { printf("gethostbyname не удалось найти %s", argv[1]); exit(-1); }
    s = socket(PF_INET, SOCK_STREAM, IPPROTO_TCP); // Создаём TCP-сокет (IPv4, потоковый, TCP)
    if (s < 0) { printf("сбой вызова сокета"); exit(-1); }
    
    memset(&channel, 0, sizeof(channel));
    channel.sin_family = AF_INET; // Семейство адресов: IPv4
    memcpy(&channel.sin_addr.s_addr, h->h_addr, h->h_length); // Копируем IP-адрес сервера из структуры hostent в sockaddr_in
...
        if (bytes <= 0) exit(0);
        write(1, buf, bytes); // Пишем принятые данные в stdout
    }
}
```

- Какие сетевые задачи решает пример файлового сервера, и почему он считается «минимальным»? ⭐
- Объясните, зачем используются htons/htonl в серверном коде и что будет при их отсутствии. ⭐⭐
- Почему сервер использует INADDR_ANY, и какие плюсы/риски у такого выбора? ⭐⭐
- Зачем включают SO_REUSEADDR и какие проблемы он помогает решить при перезапуске сервера? ⭐⭐
- Почему сервер с таким кодом плохо масштабируется при нескольких клиентах? ⭐⭐
- Какие угрозы безопасности возникают из-за того, что клиент может запросить любой файл по пути? ⭐⭐⭐
- Какие ошибки возможны из-за предположения, что имя файла «придёт за один read»? ⭐⭐⭐
- Почему клиент делает gethostbyname перед connect, и как это связывает транспортный уровень с DNS? ⭐⭐
- Как корректно обрабатывать ситуацию, когда read/write передают меньше байт, чем ожидалось? ⭐⭐⭐

### Элементы транспортных протоколов. Адресация

- Какие проблемы транспортного уровня похожи на канальный уровень, а какие принципиально отличаются? ⭐⭐
- Почему на транспортном уровне нужно явно указывать адрес получателя, а на канальном иногда «достаточно линии»? ⭐⭐
- Какие виды «аномального поведения сети» (перестановка, дублирование, задержка) особенно опасны для транспорта? ⭐⭐
- Почему наличие множества соединений одновременно усложняет буферизацию и контроль потока? ⭐⭐⭐
- Объясните принцип end-to-end контроля ошибок и почему канального контроля недостаточно. ⭐⭐⭐
- Что такое TSAP и почему «порт» — это не просто число, а точка доступа к службе? ⭐⭐
- Почему для уникальной идентификации конечной точки нужны и NSAP (IP), и TSAP (порт)? ⭐⭐
- Почему существуют «хорошо известные порты» и какие риски возникают при их использовании? ⭐⭐
- В каких случаях удобнее использовать portmapper вместо фиксированных портов? ⭐⭐
- Объясните идею inetd: почему запуск сервиса «по требованию» может быть выгоднее постоянного демона? ⭐⭐
- Какие проблемы производительности и безопасности может создать модель inetd? ⭐⭐⭐


### Установление и разрыв соединения

- Почему «установить соединение» нельзя считать простым обменом из двух сообщений? ⭐⭐
- Как задержавшийся дубликат CONNECTION REQUEST может привести к логической катастрофе (например, повторной операции)? ⭐⭐⭐
- Почему протокол обязан защищаться от дубликатов, даже если они кажутся «маловероятными»? ⭐⭐
- Какие механизмы на практике помогают отличать новый запрос от старого дубликата? ⭐⭐⭐
- Зачем в сетях ограничивают время жизни пакетов и какие подходы для этого существуют? ⭐⭐
- Объясните смысл периода T: что именно должно «исчезнуть» за это время и почему это важно. ⭐⭐⭐
- Почему нельзя бесконечно использовать маленькое пространство номеров последовательности? ⭐⭐
- Как связаны скорость передачи данных и требуемый размер пространства номеров последовательности? ⭐⭐⭐
- Почему временные метки в пакетах могут быть сложны в реализации на практике? ⭐⭐⭐
- Почему получатель «не знает историю номеров» при первом запросе соединения? ⭐⭐
- Опишите три шага «тройного рукопожатия» и смысл каждого шага. ⭐⭐
- Как тройное рукопожатие защищает от задержавшихся дубликатов CONNECTION REQUEST? ⭐⭐⭐
- Почему TCP всегда использует тройное рукопожатие, а не более короткую схему? ⭐⭐⭐
- Зачем TCP выбирает начальные номера последовательности псевдослучайно и почему «просто 0» — плохая идея? ⭐⭐⭐
- В чём риск асимметричного разрыва соединения для данных, которые «ещё в пути»? ⭐⭐
- Почему симметричный разрыв считается более безопасным и что он означает на практике? ⭐⭐
- Объясните «проблему двух армий» и как она связана с завершением соединения. ⭐⭐⭐
- Почему невозможно создать идеальный протокол согласованного разрыва по ненадёжному каналу? ⭐⭐⭐
- Какие компромиссы приходится принимать протоколам при завершении соединений в реальном мире? ⭐⭐⭐

### Прикладной уровень. Основы DNS

- Почему прикладной уровень считается «уровнем реальных приложений», и какие задачи он не может решить без нижних уровней? ⭐⭐
- Чем прикладной протокол отличается от приложения, которое им пользуется? Приведите примеры. ⭐⭐
- Объясните, почему на прикладном уровне всё равно нужны стандартизированные протоколы, даже если транспортный уровень уже «передаёт данные». ⭐⭐
- Какие риски появляются, если каждое приложение реализует «свои правила» обмена без общего протокола? ⭐⭐
- Как выбор транспортного протокола (TCP/UDP/QUIC) влияет на проектирование прикладного протокола? ⭐⭐⭐
- Почему прикладной уровень часто «знает» больше о смысле данных, чем транспортный, и как это влияет на оптимизацию? ⭐⭐⭐
- Почему людям неудобно работать с IP-адресами напрямую и какие проблемы DNS решает помимо удобства? ⭐⭐
- Объясните, почему «имя сервиса» и «IP-адрес сервиса» нельзя считать стабильной парой в современном интернете. ⭐⭐
- Почему файл hosts.txt перестал масштабироваться, и какие свойства DNS устранили эти ограничения? ⭐⭐⭐
- Как DNS помогает поддерживать миграции сервисов, балансировку нагрузки и отказоустойчивость? ⭐⭐⭐
- Почему DNS называют распределённой базой данных, и в чём отличие от централизованного справочника? ⭐⭐
- Какие классы ошибок в сети стали критичными после появления DNS (например, неверные кэши), и почему? ⭐⭐⭐

### DNS. иерархия имён, управление доменами, резолвинг

- Объясните структуру DNS-иерархии (root → TLD → домены → поддомены) и почему она похожа на адресацию в реальном мире. ⭐⭐
- Чем gTLD отличаются от ccTLD и почему обе группы существуют одновременно? ⭐⭐
- Что такое FQDN и почему абсолютное доменное имя логически заканчивается точкой? ⭐⭐
- Почему DNS-имена считаются логическими, а не отражающими физическую топологию сети? ⭐⭐⭐
- Какие организационные роли играют ICANN, реестры и регистраторы, и зачем нужна такая модель? ⭐⭐⭐
- Как ограничения длины меток и полного имени влияют на дизайн доменных структур в компаниях? ⭐⭐
- Опишите полный путь DNS-запроса от приложения до получения IP: какие компоненты участвуют и кто что делает? ⭐⭐⭐
- Что такое stub resolver и почему он обычно не выполняет полный поиск сам? ⭐⭐
- В чём разница между рекурсивным и итеративным DNS-поиском, и почему на практике используются оба? ⭐⭐⭐
- Почему корневые DNS-серверы обычно возвращают «направление дальше», а не конечный IP? ⭐⭐
- Как кэширование ускоряет DNS и какие новые проблемы оно создаёт? ⭐⭐⭐
- Что такое TTL, почему он необходим, и какие последствия у слишком большого или слишком малого TTL? ⭐⭐⭐
- Объясните, как «устаревший кэш» может приводить к недоступности сервиса даже при исправной инфраструктуре. ⭐⭐⭐
- Почему один и тот же домен может резолвиться в разные IP-адреса у разных пользователей, и как это связано с балансировкой? ⭐⭐⭐

### DNS: записи (RR), зоны и обратный поиск, безопасность и диагностика

- Что такое ресурсные записи (RR) в DNS и почему ответ — это не просто «один IP»? ⭐⭐
- Объясните назначение записей A и AAAA и когда клиенту нужны обе. ⭐⭐
- Зачем существуют записи NS и как они связаны с делегированием домена? ⭐⭐⭐
- Для чего используются MX-записи и как они влияют на доставку почты между доменами? ⭐⭐⭐
- В каких случаях применяют CNAME и какие проблемы может создать «цепочка псевдонимов»? ⭐⭐⭐
- Что такое PTR-запись и почему обратный поиск (IP → имя) технически и организационно сложнее прямого? ⭐⭐⭐
- Что означает «DNS разделён на зоны» и почему зона — это не то же самое, что домен? ⭐⭐⭐
- Как работает отравление DNS-кэша (cache poisoning) на концептуальном уровне и почему оно опасно? ⭐⭐⭐
- Что даёт DNSSEC, какие угрозы он снижает, и почему он не решает проблему приватности? ⭐⭐⭐
- Чем DNS-over-TLS и DNS-over-HTTPS отличаются от классического DNS по UDP, и что именно они защищают? ⭐⭐⭐
- Что такое QNAME-minimization и почему она снижает утечки данных о пользователе? ⭐⭐
- Почему усиление приватности DNS может приводить к укрупнению крупных резолверов и потенциальной централизации? ⭐⭐⭐
- Какие типы проблем можно диагностировать с помощью dig, которые не видно через браузер? ⭐⭐
- Объясните, как по выводу dig понять: ответ пришёл из кэша или от авторитетного сервера. ⭐⭐⭐
- Как по dig увидеть балансировку (несколько IP) и почему это не всегда означает Round-Robin в чистом виде? ⭐⭐⭐
- Какие признаки в dig помогают понять, что включена рекурсия (rd/ra) и почему это важно? ⭐⭐
- Зачем нужны glue-записи и что может сломаться, если они настроены неправильно? ⭐⭐⭐
- Почему dig полезен для анализа IPv6-готовности домена? ⭐⭐

### Электронная почта

- Почему e-mail остаётся важным приложением интернета, несмотря на мессенджеры? ⭐⭐
- Опишите архитектуру e-mail: какие роли играют MUA и MTA и где проходит граница ответственности. ⭐⭐⭐
- Почему почта хранится в mailbox на сервере и какие преимущества это даёт пользователю? ⭐⭐
- Чем отличается подача письма (mail submission) от пересылки между доменами, и почему это разделяют? ⭐⭐⭐
- Почему SMTP считают протоколом «сервер–сервер», а не «пользователь–сервер»? ⭐⭐
- Объясните структуру e-mail по RFC 5322: заголовки → пустая строка → тело. Почему это важно для совместимости? ⭐⭐
- Чем отличаются поля To, Cc и Bcc, и почему Bcc требует особого обращения при доставке? ⭐⭐⭐
- Зачем существуют поля From и Sender, и в каких сценариях они могут различаться? ⭐⭐⭐
- Как заголовки Received помогают восстановить маршрут письма и расследовать проблемы доставки? ⭐⭐⭐
- Что такое Return-Path и как он связан с сообщениями об ошибках доставки? ⭐⭐
- Зачем нужен Message-ID и какие проблемы возникли бы без уникальных идентификаторов писем? ⭐⭐⭐
- Почему исходный e-mail-формат плохо подходил для вложений и не-ASCII языков? ⭐⭐
- Какие задачи решает MIME и почему его называют «расширением без поломки совместимости»? ⭐⭐⭐
- Объясните назначение Content-Type и приведите примеры, как оно влияет на отображение письма. ⭐⭐
- Зачем нужен Content-Transfer-Encoding и почему base64 — это компромисс, а не «бесплатное решение»? ⭐⭐⭐
- Что означает multipart/* и как одно письмо может содержать одновременно текст и несколько вложений? ⭐⭐
- Какие ошибки безопасности и удобства могут возникать из-за неправильной интерпретации MIME-типов? ⭐⭐⭐

### Всемирная паутина. Браузер. Веб сервер.

- Почему веб стал «массовым интернетом», хотя интернет существовал и до него? ⭐⭐
- Как гиперссылки и идея гипертекста изменили способ работы с информацией по сравнению с «файловыми каталогами»? ⭐⭐⭐
- Почему веб-страница почти всегда состоит из множества объектов, а не из одного файла? ⭐⭐
- Почему объекты одной страницы часто загружаются с разных доменов, и какие плюсы/минусы это даёт? ⭐⭐⭐
- Чем статические страницы отличаются от динамических, и почему современный веб в основном динамический? ⭐⭐
- Какие три вопроса решает URL («что? где? как?») и почему этого достаточно для доступа к ресурсу? ⭐⭐
- Опишите типичный процесс загрузки страницы в браузере: от URL до отрисовки. ⭐⭐⭐
- Почему DNS почти всегда является первым «узким местом» в загрузке страницы, и как это проявляется? ⭐⭐⭐
- Что показывает waterfall diagram и как по нему отличить проблему DNS от проблемы сервера или сети? ⭐⭐⭐
- Почему повторное использование соединений может ускорить загрузку даже при той же пропускной способности? ⭐⭐
- Приведите примеры схем URL, которые не относятся к web (http/https), и объясните, как меняется смысл обращения. ⭐⭐
- Опишите работу «наивного» веб-сервера для статического контента и его главные ограничения при нагрузке. ⭐⭐⭐
- Почему медленный диск становится узким местом и как кэширование в памяти решает эту проблему? ⭐⭐
- Как многопоточность или пул worker’ов повышают производительность сервера, и какие новые риски они добавляют? ⭐⭐⭐
- Что такое reverse proxy и почему современная архитектура часто строится как proxy ↔ backend? ⭐⭐⭐
- Почему reverse proxy особенно полезен для статических объектов, даже если динамика обслуживается отдельно? ⭐⭐

### Протоколы HTTP и HTTPS

- Почему HTTP построен как протокол «запрос–ответ», и какие ограничения это накладывает на приложения? ⭐⭐
- Что добавляет TLS в HTTPS (шифрование, целостность, аутентификация) и какие угрозы он закрывает? ⭐⭐⭐
- Чем HTTP/1.x, HTTP/2 и HTTP/3 отличаются по транспорту и почему это влияет на задержки и параллелизм? ⭐⭐⭐
- Почему HTTP давно стал протоколом не только браузеров, но и API/облаков/сервис–сервис взаимодействия? ⭐⭐
- Объясните различие между GET и POST с точки зрения семантики и типичных сценариев. ⭐⭐
- Зачем существует метод HEAD и когда он полезнее GET? ⭐⭐
- Как интерпретировать классы кодов 2xx/3xx/4xx/5xx и почему это важно для автоматических клиентов? ⭐⭐
- Как cookie позволяют HTTP «имитировать состояние» и какие риски приватности из этого следуют? ⭐⭐⭐
- Объясните, как работают ETag и условные запросы (If-None-Match / If-Modified-Since) и почему они экономят трафик. ⭐⭐⭐
- Почему модель «1 запрос → 1 соединение» (HTTP/1.0) стала проблемой при росте числа объектов на странице? ⭐⭐
- Что такое persistent connections (keep-alive) и какие компромиссы появляются при их использовании? ⭐⭐⭐
- Что такое pipelining в HTTP/1.1 и почему он оказался сложным/ограниченным на практике? ⭐⭐⭐
- Почему параллельные TCP-соединения могут ускорять загрузку, но считаются «агрессивными» к сети? ⭐⭐⭐
- Что такое мультиплексирование в HTTP/2 и почему оно улучшает загрузку страниц по сравнению с HTTP/1.1? ⭐⭐⭐
- Как приоритизация запросов в HTTP/2 влияет на «ощущаемую скорость» загрузки страницы? ⭐⭐⭐
- Почему двоичный формат и сжатие заголовков дают выигрыш, даже если данные страницы не очень большие? ⭐⭐
- Объясните идею server push и приведите пример, когда он полезен и когда может навредить. ⭐⭐⭐
- Как HTTP/2 уменьшает head-of-line blocking на уровне запросов, но не устраняет его полностью на уровне TCP? ⭐⭐⭐
- Почему HTTP/3 использует QUIC поверх UDP вместо TCP, и какие проблемы веба это решает? ⭐⭐⭐
- Что означает «мультиплексирование потоков без блокировки очереди между потоками» в QUIC? ⭐⭐⭐
- Почему установка соединения в QUIC может быть быстрее по RTT, и где это особенно заметно? ⭐⭐
- Как клиент и сервер договариваются о версии HTTP (ALPN, Alt-Svc) и почему HTTP/3 «обнаруживается» иначе? ⭐⭐⭐


### Передача аудио и видео. Сети доставки контента. P2P

- Чем потоковая передача отличается от обычной загрузки файла с точки зрения требований к сети? ⭐⭐
- Почему для VoIP критичнее задержка, чем пропускная способность, а для видео часто наоборот? ⭐⭐⭐
- Что такое джиттер и почему он может быть более разрушительным для качества, чем средняя задержка? ⭐⭐⭐
- Почему рост пропускной способности и вычислительной мощности стал ключом к массовому стримингу? ⭐⭐
- Объясните теорему Найквиста и её связь с выбором частоты дискретизации звука. ⭐⭐⭐
- Что такое перцепционное кодирование и почему оно позволяет «выбрасывать данные», почти не ухудшая качество? ⭐⭐⭐
- Чем отличаются I-, P- и B-кадры в видеокодеках и почему это резко снижает объём данных? ⭐⭐⭐
- Почему кодирование видео обычно сложнее декодирования, и как эта асимметрия влияет на индустрию? ⭐⭐
- Почему современный интернет всё больше похож на систему доставки контента, а не на сеть «диалогов»? ⭐⭐
- Объясните закон Ципфа («длинный хвост») и как он влияет на стратегию кэширования и CDN. ⭐⭐⭐
- Как CDN снижает задержку и магистральный трафик за счёт размещения копий ближе к пользователю? ⭐⭐
- Как DNS redirection помогает выбрать ближайший узел CDN и почему для этого часто нужен короткий TTL? ⭐⭐⭐
- Почему у CDN часто есть «двухуровневый» выбор (регион → сервер в кластере) и что это даёт? ⭐⭐⭐
- Как происходит заполнение кэша CDN от origin-сервера, и почему это похоже на распространение «деревом»? ⭐⭐
- Почему HTTPS усложняет работу CDN и зачем CDN нужен сертификат/доверие? ⭐⭐⭐
- Чем принципиально отличается CDN от P2P по тому, как масштабируется пропускная способность системы? ⭐⭐⭐
- Почему централизованный каталог Napster был удобен, но уязвим технически и юридически? ⭐⭐⭐
- Как работает распределённый поиск в стиле gossip (как в Gnutella) и почему он плохо масштабируется? ⭐⭐⭐
- Кто такие free-riders (leechers) и почему они разрушают эффективность P2P-сети? ⭐⭐⭐
- Почему в P2P возникает проблема «засорения» (pollution/fake content) и как она влияет на доверие к сети? ⭐⭐⭐
- Как BitTorrent делит файл на chunks и почему это ускоряет распространение контента? ⭐⭐
- Как swarm, tracker и DHT помогают пирам находить друг друга и что произойдёт, если один из элементов исчезнет? ⭐⭐⭐
- Объясните стратегию tit-for-tat: как она мотивирует раздачу и повышает скорость сети. ⭐⭐⭐
- Почему проверка хешей каждого сегмента повышает надёжность BitTorrent даже при наличии злоумышленников? ⭐⭐⭐
