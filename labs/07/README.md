...
- [Лабораторная работа №7](#лабораторная-работа-7)
  - [Задания](#задания)
    ...
    - [Задание 1: подготовка образа реального Flash-накопителя в ОС Linux — 50 баллов](#задание-1-подготовка-образа-реального-flash-накопителя-в-ос-linux--50-баллов)
      - [Цели](#цели)
      - [Шаг 1. Подключить флешку и определить устройство](#шаг-1-подключить-флешку-и-определить-устройство)
    ...
    - [Задание 3: Реализация собственной одноуровневой файловой системы и её монтирование через FUSE — 200 баллов](#задание-3-реализация-собственной-одноуровневой-файловой-системы-и-её-монтирование-через-fuse--200-баллов)
      - [1. Цель задания](#1-цель-задания-1)
      - [2. Требования к файловой системе](#2-требования-к-файловой-системе)
        - [2.1. Заголовок (Superblock)](#21-заголовок-superblock)
        - [2.2. Таблица файлов (Directory Entries)](#22-таблица-файлов-directory-entries)
        - [2.3. Область данных (Data Area)](#23-область-данных-data-area)
      - [3. Ограничения](#3-ограничения)
      - [4. Требования к приложению FUSE](#4-требования-к-приложению-fuse)
        - [Минимальный набор реализуемых операций FUSE:](#минимальный-набор-реализуемых-операций-fuse)
      - [5. Форматы команд](#5-форматы-команд)
        - [Создание образа](#создание-образа)
        - [Копирование файла в образ](#копирование-файла-в-образ)
      - [6. Пример вывода](#6-пример-вывода)
      - [Бонусное задание: поддержка фрагментированных файлов — 50 баллов](#бонусное-задание-поддержка-фрагментированных-файлов--50-баллов)
...
#### Шаг 1. Подключить флешку и определить устройство
...
### Задание 3: Реализация собственной одноуровневой файловой системы и её монтирование через FUSE — 200 баллов

В этом задании вам предлагается разработать собственную простую файловую систему,
созданную в виде одного бинарного файла-образа, и реализовать пользовательский модуль FUSE,
позволяющий монтировать этот образ в реальную файловую систему Linux.
После монтирования содержимое кастомной ФС должно быть доступно стандартными командами
(`ls`, `cat`, `cp`, `stat`, `file`, и т.д.).

Файловая система является **одноуровневой**, т.е. допускает только один каталог — корневой `/`.
В нём могут располагаться файлы, но **подкаталоги не поддерживаются**.

#### 1. Цель задания

Студент должен:

- спроектировать собственный формат файловой системы;
- реализовать хранение файлов внутри образа;
- создать монтируемую файловую систему на основе FUSE;
- обрабатывать системные вызовы уровня `getattr`, `readdir`, `open`, `read`, `write`;
- понять принципы VFS и интеграции пользовательской ФС с ядром Linux;
- освоить хранение данных в виде непрерывных блоков (contiguous allocation);
- (бонус) реализовать поддержку разрывных файлов (фрагментацию).

#### 2. Требования к файловой системе

Образ файловой системы представляет собой один файл, например:

```txt
myfs.img
```

Внутренняя структура образа определяется студентом. Минимально допустимая структура включает три части:

##### 2.1. Заголовок (Superblock)

В заголовке должны храниться как минимум:

- сигнатура файловой системы (например, `"MYFS1\0"`);
- версия формата;
- максимальное количество файлов;
- размер блока в байтах;
- количество блоков в разделе данных;
- смещение начала таблицы метаданных;
- смещение начала области данных.

Студент самостоятельно выбирает формат и типы данных. Должна быть реализована сериализация/десериализация заголовка.

##### 2.2. Таблица файлов (Directory Entries)

Файловая система одноуровневая, поэтому таблица представляет собой линейный массив записей:

Каждая запись должна содержать:

- имя файла (UTF-8 или ASCII, длина <= 255 символов);
- размер файла (до 2 ГБ);
- смещение начала данных в блоках;
- количество блоков, занимаемых файлом;
- признак «занято/свободно»;
- атрибуты (только тип: файл).

Файлы **не могут иметь одинаковые имена**.

Таблица должна позволять:

- находить запись по имени;
- добавлять запись (создание файла);
- удалять запись;
- изменять размер файла (при необходимости, опционально);
- отмечать свободные блоки.

##### 2.3. Область данных (Data Area)

Содержит сырой контент файлов.

В рамках обязательной части задания требуется использовать **непрерывное размещение**:

- файл занимает строго последовательные блоки;
- при создании файла система ищет свободный непрерывный диапазон;
- при чтении файла достаточно знать смещение и длину.

Это упрощает реализацию и делает структуру максимально прозрачной.

#### 3. Ограничения

1. Файловая система одноуровневая — разрешён только корневой каталог `/`.
2. Максимальный размер файла — **2 ГБ**.
3. Файлы размещаются **непрерывно**, без фрагментации (см. бонус).

#### 4. Требования к приложению FUSE

Студент должен написать программу вида:

```bash
myfs-mount <image_path> <mount_point>
```
...
##### Минимальный набор реализуемых операций FUSE:

* `getattr` — получение атрибутов файла/каталога;
* `readdir` — вывод содержимого корневого каталога;
* `open` — открытие файла;
...
Для проверки предусмотрите следующие команды:

##### Создание образа

```bash
myfs-mkfs myfs.img --size 128M --max-files 256
```

##### Копирование файла в образ

```bash
myfs-cp host_file myfs.img:/filename
```
...
$ stat mnt/image.bin
  Size: 67108864    Blocks: 16384    IO Block: 4096 regular file
```

#### Бонусное задание: поддержка фрагментированных файлов — 50 баллов

В обязательной части файл занимает непрерывную последовательность блоков.

В бонусной части студент может:

- заменить поле “смещение + длина” на массив структур:
...
- хранить цепочку extent’ов в директории;
- читать файл как последовательность данных из нескольких несмежных областей.

Это повысит производительность и расширит возможности ФС.
